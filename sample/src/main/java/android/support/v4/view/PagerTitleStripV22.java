...
public class PagerTitleStripV22 extends ViewGroup implements ViewPager.Decor {
  ...
  void updateText(int currentItem, PagerAdapter adapter) {
    ...
    mCurrText.setText(
        adapter != null && currentItem < itemCount ? adapter.getPageTitle(currentItem) : null);
  ...
  void updateTextPositions(int position, float positionOffset, boolean force) {
    ...
    final int maxTextHeight =
        Math.max(Math.max(alignedPrevHeight, alignedCurrHeight), alignedNextHeight);
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    final int childWidthSpec =
        MeasureSpec.makeMeasureSpec((int) (widthSize * 0.8f), MeasureSpec.AT_MOST);
    final int childHeightSpec = MeasureSpec.makeMeasureSpec(childHeight, MeasureSpec.AT_MOST);
  ...
  private class PageListener extends DataSetObserver
    ...
    @Override public void onAdapterChanged(PagerAdapter oldAdapter, PagerAdapter newAdapter) {
